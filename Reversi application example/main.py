...
def loadImages(path, size):
    """load an image into the game and scale image"""
...
class Reversi:
    ...
    def update(self):
        ...
        self.grid.player1Score = self.grid.calculatePlayersScore(self.player1)
        self.grid.player2Score = self.grid.calculatePlayersScore(self.player2)
        if not self.grid.findAvailMoves(self.grid.gridLogic, self.currentPlayer):
            self.gameOver = True
            return
...
class Grid:
    ...
    def calculatePlayersScore(self, player):
        score = 0
    ...
    def swappableTiles(self, x, y, grid, player):
        ...
        if len(surroundCells) == 0:
            return []
        swappableTiles = []
        for checkedCell in surroundCells:
            checkX, checkY = checkedCell
